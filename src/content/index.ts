export const navLinks = [
  { title: 'Recipes', href: '/' },
  { title: 'FAQ', href: '#' },
  { title: 'Blog', href: '#' },
]

export const featuresText = [
  { title: '10K+', description: 'Satisfied Costumers All Great Over The World' },
  { title: '4M', description: 'Healthy Dishes Sold Including Milk Shakes Smoothies' },
  { title: '99.99%', description: 'Reliable Customer Support We Provide Great Experiences' },
]

export const downloadText = [
  'Premium quality food is made with ingredients that are packed with essential vitamins, minerals.',
  'These foods promote overall wellness by support healthy digestion and boosting immunity',
]

export const socialLinks = [
  { title: 'Instagram', href: '#', icon: 'inst' },
  { title: 'Linkedin', href: '#', icon: 'in' },
  { title: 'Facebook', href: '#', icon: 'facebook' },
  { title: 'Twitter', href: '#', icon: 'twit' },
]
